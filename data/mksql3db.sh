...
do
    ...
    stops.txt)
	    ...
	    tail -n +2 "stops.txt" | sort -k6 -g -t, | grep -n '^' - | sed -e '/^$/d' | sed -e 's/,\ */,/g' | sed -e 's/_merged_[0-9]*//g' | sed -e 's/\([0-9]\):/\1,/g' > $tmpfile
	    (
    ...
    stop_times.txt)
	    ...
	    tail -n +2 "stop_times.txt" | sed -e '/^$/d' | sed -e 's/,\ */,/g' | sed -e '/^\s*$/d' | sed -e 's/_merged_[0-9]*//g' | sed -e 's/\,\ \([0-9]\):\([0-9][0-9]\):\([0-9][0-9]\)/\,0\1\2\3/g' | sed -e 's/\,\([0-9]\):\([0-9][0-9]\):\([0-9][0-9]\)/\,0\1\2\3/g' | sed -e 's/://g' > $tmpfile
